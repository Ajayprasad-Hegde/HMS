...
public class Util
{
	...
	public static void launchBrowser(String browser)    					// Launching browser
	{
		...
		if (browser.equals("chrome"))
		{
			...
			test_cases.Base.driver = new ChromeDriver();
		...
		test_cases.Base.driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(10));
		test_cases.Base.driver.manage().window().maximize();
	}
	...
	{
		TakesScreenshot ts = (TakesScreenshot) test_cases.Base.driver;
		File sf = ts.getScreenshotAs(OutputType.FILE);
		File df = new File ("C:\\Ajay\\Java_Folder\\Java_Eclips\\HMS_MAVEN"
				+ "\\Failed Test Screenshots\\"+fileName+dateTime()+".jpg");
	...
	public static boolean imageCompare(String expectedImagepath, WebElement ele) throws IOException    // Image compare
	{
		...
		BufferedImage actualImage = ashot.takeScreenshot(test_cases.Base.driver, ele).getImage();
